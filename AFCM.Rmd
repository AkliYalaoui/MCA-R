...
Pour pouvoir transformer notre dataset à un tableau disjonctif complet, il faut d'abord transformer les colonnes au type factor :
...
On a remarqué que la variable nationality possède un nombre de modalité important par rap^port au autres variables, donc on va l'éliminer : 
```{r}
print(length(levels(data$Nationality)))
...
On a 125 individus, ici nous allons considérer que les modalités avec une fréquence moins de 15% sont trés faible, donc on va éliminer ces modalités, nous avons opter pour cette option pour alléger un peu le nombre de colonne [ on avait 120 modalités] 
...
```
On remarque que pour avoir un pourcentage d'interie d'au moins 80% il faut prendre les 20 premiers axes factoriels :
...
Une variable contribue bien à la construction du premier axe si so contribution absolue est supérieur à son poids : 


```{r}
contrib <- as.data.frame(acm$col$contrib)
coord <- as.data.frame(acm$col$coord)
...
for(i in 1:ncol(df)){
      ...
      if(contrib$`Dim 1`[i] >= fs*100 && coord$`Dim 1`[i] > 0){
...
for(i in 1:ncol(df)){
      ...
      if(contrib$`Dim 2`[i] >= fs*100 && coord$`Dim 2`[i] > 0){
        print(c(colnames(df)[i],"+"))
      }
      if(contrib$`Dim 2`[i] >= fs*100 && coord$`Dim 2`[i] < 0){
        print(c(colnames(df)[i],"-"))
