...
On a remarqué qu'il y a des valeurs manquantes dans nos données, on va les détecter pour décider comment on va remédier à ce problème.
...
```
On conclut qu'il n'y a pas de valeurs de type NA, donc on vérifie les chaines de characters vide : 
```{r}
for (i in 1: ncol(data)){
...
On aura besoin d'une fonction qui va nous donner des graphes pour chaque variables
...
```{r}
Cat_plot(data$Age,"La répartition de l'age dans cette étude")
...
On a 125 individus, ici nous allons considérer que les modalités avec un effectif moins de 5 sont trés faible, donc on va éliminer ces modalités, nous avons opter pour cette option pour alléger un peu le nombre de colonne [ on avait 94 modalités] 
...
Maintenant on a que 83 modalités , on va effectuer une AFC sur le tableau disjonctif complet : 
...
Un profile ligne (colonne) contribue bien à la construction du premier axe si sa contribution absolue est supérieue à son poids : 
...
On calcule la distance khi 2 entre chaque 2 profile colonnes : 
...
Ici nous cherchons à faire une AFC entre les 2 questions les plus pertinentes, donc nous allons faire une recherche exhaustive pour les détecter [nous allons prendre celles qui donnent des trés bon résultat lors du test de KHI 2] :
...
On remarque qu'avec seulement les 2 premiers axe on a une très très bonne représentation
...
On déduit que le premier axe est à effet taille, il caractérise les étudiants qui sont dans les programmes FC et IYO représenté par les individus qui ont un age inférieure à 20 ans
...
Aussi le 2ème axe est à effet taille, il caractérise les les étudiant qui sont dans le programme Language représenté par les étudiants agés plus de 36 ans
...
# Conclusion : 
  ...
  - Et Ceux qui ont un age compris entre 31 et 35 ans suivent le programme PM.
